...
const List = ({ list }) => {
  ...
  const searchResultHandler = (event) => {
    ...
    if (enteredName !== "") {
      ...
      setSearchedName(
        filteredRegionList.filter(
          (item) => item.name.startsWith(correctedName) === true
        )
      );
    } else {
      setSearchedName();
  ...
  if (searchedName !== undefined) {
    if (searchedName.length === 0) noResult = true;
    else finalList = searchedName;
  } else {
    finalList = filteredRegionList;
  }
  console.log("run from list");
  return (
    <>
      <Card>
      ...
      {noResult && (
        <p className={styles["no-result-para"]}>
          No country with this name in selected region.
        </p>
      )} 
